...
def test_csv_and_recorder_agree_on_the_same_history(tmp_path: Path) -> None:
    ...
    from_db = run(read_recorder(database, register=REGISTER), load("no/tensio"), OSLO)
    from_csv = run(read_csv(directory), load("no/tensio"), OSLO)
...
def test_meta_json_supplies_the_zone(tmp_path: Path) -> None:
    ...
    zone, source = resolve_tz(None, history, "no/tensio")
...
def test_meta_json_window_min_overrides_the_preset(tmp_path: Path) -> None:
    ...
    result = run(read_csv(directory), load("no/tensio"), OSLO)
