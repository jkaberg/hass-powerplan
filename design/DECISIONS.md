...
### D-0670 · A battery takes four commands; one vocabulary runs every battery row

A battery's plan slot is self-use (`None`), hold (`0`), or charge/discharge (`±w`); INV-30 is read for a battery. One `battery` kind runs a `BatteryVocabulary` row per integration, each listing per command the levers to write (option, number, switch, press, action, expiring), a prerequisite, the vendor's optimiser, a cadence and a witness. The seven built profiles become rows, and rows follow for SolarEdge, Fox ESS, Fronius, Marstek, SAJ, Sofar, Sessy, sonnen, E3/DC, Solis, Tesla, Deye and Sunsynk via Solarman, Growatt, Victron, HomeWizard and Sungrow's YAML package. "Pick entities by hand" is built; a battery with its own self-use gets a `self_use` counterfactual. At 0 W three built profiles sent self-use while D5 counted the slot idle, so a battery held for a capacity window could arrive empty. A survey of 44 integrations found six levers and no seventh. Affects HLD §1.2, §6.4, INV-30, §7.8, §9; D4 §4.2, §5.9, §6.6, §9 39-46, §10, §11; D5 §5.8; D6 §5.3; D8 §5.2, §5.9; D11 §3, §5.3.
**Rejected:** see PLAN §7 dec. 44 and each LLD's §11: 0 as the release, one kind per shape, curtailment in v1.0, an idle counterfactual, Deye's programs deferred, Victron by limits only.

### D-0671 · The battery's four commands in code

**Kind.** `core/loads/kinds/battery.py` replaces `battery_mode.py`. It sends `Role.BATTERY_COMMAND`: `self_use`, `hold`, `charge:W`, `discharge:W`, or bare `charge`/`discharge` where the inverter sets the power; `gate.same` compares the word exactly and the watts within tolerance. `Grant.answer` carries what the plan said about the slot. A planned 0 follows the measured surplus and never discharges; a bare-number battery (no self-use of its own) is balanced by the walk in a free slot.
**Profiles.** `BatteryVocabulary` rows over `Lever`s (option, number, switch, press, action), each value an `Expr` (power, signed, reserve, depth, SoC rounded up, the entity's maximum). A row reads the command back from a status sensor where it has one, else from the first command whose checkable levers all hold; `call_in(write, view)` sends only levers not already at value. The seven built profiles become rows, plus `homewizard`. The type materialises `battery_commands`, `battery_power`, `can_hold` and `self_use` from the row.
**Planner.** D5 simulates a free slot as self-use and holds the cheapest free slots before a discharge the drain would starve. A committed charge slot with nothing left to charge is a hold, never `None`.
**Accounting and tests.** D11 gets `BATTERY_SELF_USE` and `SelfUseShadow`; D9 gets `battery_holds_for_peak` in three variants.
Choices the LLD left: self-use levers write the row's constants, and the command's INV-26 prior is the command read back before the first write (so HomeWizard's `predictive` returns as the row's `vendor` lever). The hold's floor is written once on entry, not re-raised as the sun fills the battery. A battery's floor counts as violated only 1 point below the reserve, since an inverter set to the reserve rests on it; otherwise a grid charge flapped 334 writes a day. `SnapshotSchema` 12. Affects D4 §4.2, §5.9, §6.6; D5 §5.8; D6 §5.3; D9 §5.3; D11 §5.3, §5.9.1.
**Rejected:** three kinds each taught a hold - the hold, prerequisite and optimiser written three times, with more shapes coming.

### D-0672 · Mode then power: six rows, and the repair

Rows `solaredge_modbus_multi`, `foxess_modbus`, `fronius_modbus`, `marstek_modbus`, `saj_h2_modbus` and `solax_modbus_sofar`, and `sigen` with its charge and discharge limits as the power. A row claims a device only when its first control is present, which separates two rows on one platform (`solax_modbus` carries SolaX and Sofar). `Expr.PERMILLE` gives SAJ's power as per mille of the rate; `Bind.plain` binds a unitless number. `battery_control_off` is raised when a row with a prerequisite can't read its command for 15 minutes or the load is unhealthy. From the sources: SolarEdge's charge also sets *AC Charge Policy* to Always Allowed, and its *Storage Default Mode* is provisioned to Maximize Self Consumption beside the one-hour timeout so a lapse lands on self-use (INV-64); Sigen charges Grid First and discharges ESS First; Sofar gets no expiring lever and relies on its own minimum SoC; Marstek's prerequisite is enabling its control entities; Fox ESS's and Marstek's power sensors have an undocumented sign and aren't bound. Affects D4 §5.9, §9 42; D8 §5.9, §9 42.
**Rejected:** Sofar's passive timeout now - two more roles and a second press for one row's housekeeping.

### D-0673 · Power through an action or a number: five rows

Rows `marstek_local_api`, `sessy`, `sonnenbatterie`, `e3dc_rscp` and `solis_modbus`, with `Lever.check_only` (read back, never written: how an action's command is read back from a sensor), `Lever.untargeted` (Solis's dispatch schema refuses a target), `Bind.negate` and `Expr.POWER_TEXT`. From the sources: sonnen's power reads positive discharging, so it's negated; `charge_battery` takes a string and needs local-API write access, its prerequisite; `charge_battery(0)` turns the override off, so the hold is manual mode at 0. E3/DC holds with power mode 1 (idle), one action with a sensor to read back, at the cost of the sun not filling it while held. Sessy's setpoint is positive discharging. Marstek's AI mode, Sessy's Dynamic strategy and sonnen's optimizing are `vendor` levers, restored on release (INV-26). Affects D4 §5.9, §9 43.
**Rejected:** holding E3/DC with a discharge limit of 0 - a second action to undo and a binary sensor to read back.

### D-0674 · The SoC floor: eight rows, and one lever over six entities

Rows `tesla_fleet`, `teslemetry`, `tessie` (one data shape), `tesla_custom`, `solarman` (Deye, Sunsynk), core `fronius`, core `growatt_server` and `solis_cloud_control`: inverter-set power, no discharge command, and core Fronius no charge command either. `RoleBinding.also` writes more entities with the same value, stored in the subentry and sent as one chain; `Bind.family` binds a numbered set. `Expr.TARGET` is the charge target, never below the reserve. From the sources: Tesla charges by raising *Backup reserve* to the target with grid charging on (*Autonomous* is its vendor mode, its power negated); Deye's power is the inverter's and its max charging current is left alone (in amps, voltage unknown); core Fronius clamps to 0-100 %, so it can hold but not force; Growatt uses only the token API, since the classic login locks accounts for a day; Tesla's five-minute cadence is an estimate. D9's `battery_holds_for_peak_floor` runs a floor row. Affects D4 §5.9, §9 44; D9 §5.3.
**Rejected:** `tesla_custom`'s own powerwall-mode select - one integration's trick, where the floor is the same lever everywhere.

### D-0675 · Victron: a setpoint to charge, a limit to discharge

`victron_gx` and `victron_mqtt` share one row shape; `victron` is the Modbus integration. Charge sets the grid setpoint to `grid_for(+W)` (measured grid − measured battery + W); discharge sets the setpoint to 0 and the discharge limit to W; hold sets the limit to 0; self-use sets the setpoint to 0 and the limit to its maximum (−1 reads as no limit, handed back). Each command switches Dynamic ESS off where its select is on the same device. A charge reads back as `grid_for`'s inverse, so a moving house is a deviation and the next tick writes a fresh setpoint (INV-22). Discharge by the limit rather than a setpoint gives the same energy and reads back without the meter. The Hub4 device carries only the overrides, so the flow asks for SoC, battery power and grid from other devices. The Modbus integration exposes the volatile override read-only, so it writes the ESS setting (register 2700) at most once a minute. Tolerance 100 W and cadences are estimates (no house here has one). Affects D4 §5.9, §9 45, §11.
**Rejected:** reading the grid from powerplan's site meter - the provider's write sees only the device's view, and ESS regulates to its own meter.

### D-0676 · Entities on no device: one picker, then the match
...
### D-0682 · Denmark: a household is billed `flex`, and Datahub fills what elpris.dk can't price

elpris.dk lists each charge as `flex` (24 hourly prices) and `fix` (no hours; the flat rate of a meter not settled by the hour). The adapter reads the area's charge code, its `flex` records still in force, and Datahub's rows for the code from the charge's `validFrom`. Where elpris.dk has no usable `flex` hours - 14 areas list only `fix`, whose price is one hour of Datahub's table (Zeanet: the night rate), and Elinord's 43300 is two companies' rows merged - Datahub's rows are the tariff. Datahub is asked by code alone and `owned` picks the company: the code's only owner, else the one named as elpris.dk names it, case and punctuation aside, the area-suffixed one first ("Konstant Net A/S - 151"). 34 of 34 areas fetch on 2026-09-26, against 17. Affects D13 §5.9, §5.11.
**Rejected:** reading `fix` as a flat tariff - Zeanet's would bill 0.0719 DKK/kWh at 18:00 where Datahub says 0.2803. Refusing an area whose elpris.dk rows fail - Datahub, the official register, has them.

### D-0683 · Australia's plans come from Energy Made Easy; a retailer without one says so

The CDR register gives 33 of 84 brands their own host as `publicBaseUri`, and every one answered 404 for `…/energy/plans` on 2026-09-26; the AER publishes the same plans at `cdr.energymadeeasy.gov.au/<slug>`. `cdr_energy.EME_SLUGS` maps 26 of them by brand name, each probed that day for a plan carrying the brand's name - Energy Made Easy answers 200 with no plans for any slug, so only a plan confirms one. The other seven list none there and keep their host. A retailer whose source lists no residential electricity plan gets `tariff_no_plans` in the flow, not the "try again later" of `tariff_source_unreachable`; the canary counts it only when every brand of a source lists none. The canary compares two tiers' copies of a company on products of one kind - dwelling, main fuse, region, a 16 A apartment first - and flags it only when no pair agrees; a source that lists by postcode alone (CWaPE, URDB) is asked for a probe postcode. That probe found CompaCWaPE and BruSim answering JSON-LD collections (`hydra:member`) where the adapter reads a plain list, which broke every Belgian postcode in the flow: the adapter now asks for `Accept: application/json`. Affects D13 §5.5, §5.7.
**Rejected:** falling back to Energy Made Easy on a 404 - two requests per brand for a path that fails a third of the list. Deriving the slug from the host - `authncdr.energyaustralia.com.au` is `energyaustralia`, `public.mydata.cdr.originenergy.com.au` is `origin`.

### D-0684 · The canary keeps one dashboard issue

Every nightly run rewrites the body of one issue, "Tariff test dashboard", with the run's report and a link to it, found or not, and creates it only when no issue has that title; a closed one is edited, not reopened. A run whose canary crashed before it reported still rewrites it, and fails. The canary reports an adapter that raises something other than `SourceError` as a finding (`crashed: …`) and goes on. Affects D9 §5.15, D13 §5.7.
**Rejected:** an issue per day (D-0559's) - 123 findings a night repeat until someone fixes them; the dashboard shows what is broken now, and the history is the run log.

### D-0685 · The ladder projects from the measured total, never a plan or a forecast

`Budget.projected_kwh = used + P_smooth × t_rem` at every baseline confidence; `budget()` loses `controlled_planned_kwh`, `tick()` stops summing plans and `Plan.kwh_between`, its only caller gone, is removed. The baseline keeps its reserve half of D-0319: σ from D10's residual when confident, never under the floor. D-0319 had switched the projection to `used + Σ plan envelopes + ∫ baseline` when the baseline was confident, against INV-38, INV-62 and D6 §5.4's own table, and four consumers - the ladder, `cap_for_projection`, the group's scarcity test, the live warning - still read it as the measured number. On the reference house it counted a banked floor's cap as a draw, the room D-0629 had already handed the EV for it a second time, and a charging plan for a car that wasn't plugged in: fifteen escalations in two nights, stage 3 seven hours in a row with the house at 17–37 % of the ceiling. Every plan-driven grant is capped at the budget (D6 §5.3), the argument D-0627 made for the warning. `sensor.<site>_window_projected` keeps the ladder's number and gains `expected_kwh`, D7 §5.4's current-window expected. Affects HLD §6.6; D6 §1, §2, §3, §4, §5.4, §9 3, 30, §10, §11; D7 §5.1, §5.4, §9 27; D8 §5.5, §9 44; D9 §5.3.
**Rejected:** prorating the planned draw in `kwh_between` and skipping absent loads - the ladder would still read a forecast, and the reference seed's +1.6 kW at 21:00 (D-0687) would shed for it. Projecting what this tick's grants will draw - circular, since the stage feeds the walk.

### D-0686 · An idle on/off thermostat reserves its plan's draw, not a margin

D6 §5.2: an idle on/off thermostat (a `mode` floor, a resistive tank) reserves `_planned_draw_w` for the slot - the standing loss D5 reserves for it (D-0629) - and 0 without a plan; granted or on, it reserves its nameplate as before (D-0169). D-0168's measured-plus-500 W margin is for a modulating thermostatic load only. `LoadView.thermostatic` is true for every `setpoint` and `mode` kind, so on the reference house five idle floors and the tank each held 500 W: Σ reserved 4 424 W against `P_allow` 4 964 W at 4.7 kWh, the entrance floor refused at every hour's start and the EV left nothing. A relay that closes on its own is on the next tick (≤ 10 s) at its nameplate, ≈ 3 Wh of surprise. Affects HLD §6.6; D6 §5.2, §9 31, §11; D-0168.
**Rejected:** a margin per thermostat - they don't start together, and at 4.7 kWh the margins were 74 % of the allowance. One site margin for the largest thermostat able to start - 1 920 W on the reference snapshot, still refusing the entrance, and it needs a per-kind "could start" test.

### D-0687 · One reader places every register statistics row

`providers/meters/recorder.py::async_register_rows` is the one reader of the import register's statistics for D10 and D8 (D2's period seed keeps `async_register_history`, the hourly table alone), the 5-minute table within `RECENT_DAYS` (each row at `S + 5 min − min(c, 5 min)`) as well as the hourly one (D-0352's `S + 1 h − min(c, 1 h)`). D10's seed and D8's fixed-price saving read through it; `recorder_baseline._statistic_rows` keeps only the loads' `mean` rows (D-0483) and `async_site_register_kwh` goes. D10's own reader placed every `sum` at S, where HA's `sum` is the register after the last report inside the period (`sensor/recorder.py`), so on a register reporting every 10 s the meter ran a period early against the loads: the reference baseline held 2 990 W at 21:00 against 1 376 W measured, with two negative bins; re-seeded offline with the core's own code, S-placement gives 2 149 W and six negative bins, correct placement 1 374 W and none. The seed logs and stores `lag_h` - 0 on the reference house once rows are placed right - and `forecasts.seed_version` 3 re-seeds once. D-0352 rejected the 5-minute table for want of a reported case; D10's seed is that case. Affects HLD §6.3; D3 §5.11, §9 22; D10 §5.2, §9 20; D8 §5.16.
**Rejected:** placing the `sum` at the row's end inside `_statistic_rows` - wrong for a latched register, and two readers of one register already diverged once. Letting the 28-day half-life wash the error out - every new install seeds it again.

### D-0688 · A plan that doesn't cover the next hour is replaced; outside a plan is `None`

D5 §5.9 adopts the new plan when `old.slots[-1].end < now + 1 h`, whatever `h` says (INV-32), and `Plan.cap_w` answers `None` for an instant outside every slot (INV-30), the load's reason naming the plan that ran out. D-0253 replaces a spent plan only when the new one has an active slot, which `best_save` never has, and on a flat curve both cost 0: the reference house kept three floors' plans past their last slot for 20 hours and read the instant after as "stand still", the TV room *planned idle* 0.4 K under target. Affects HLD INV-30, INV-32; D5 §4, §5.9, §9 30, §11.
**Rejected:** only the `cap_w` answer - the status and the calendar would keep publishing a plan from three days ago. `0` outside a plan - for a relay or setpoint load it holds the load off until a floor breaks. A TTL - coverage is one, in the plan's own time.

### D-0689 · Three read-back deviations in a row: `not_following`

A run of three read-back deviations on one role makes the load `not_following` on `sensor.<load>_health`, a state beside `ok`, `transient` and `unhealthy` that isn't a failure: the load stays in allocation and its decisions stay against the read-back (INV-22). `GateState.deviations` counts the run and resets on a match, the sensor publishes `deviations`, `last_deviation_at` and the role (the attribute D8 §5.5 already listed), and `device_unhealthy` carries it under `not_following:<load>`. The counter existed and nothing read it: the reference house's heat pump 1 took 2 of 11 setpoint writes over five hours with its health at `ok`. D4 §8's "re-issued next tick" becomes "when the gate next allows" - 900 s for a heat pump. Affects HLD INV-22; D4 §5.10, §8, §9 47, §11; D8 §5.5, §2 notifications, §9 45.
**Rejected:** counting deviations as failures - `unhealthy` takes the load out of allocation. The attribute alone - nobody opens it.

### D-0690 · A step the period has passed isn't defended

D2 §5.4: while a chosen target is below the metric a period whose metric can't fall has already reached (`mean_top_n` or `max` over a month; not `rolling_months`), the ceiling defends `auto`'s answer, the reached step, with reason `unreachable_target` and advice `target_unreachable` dated the period's end; the select keeps the choice, which binds from the next period's first window. `feasible(T)` is 0 there already (§5.6) and the step's fee can't change until the metric passes its bound: the reference house held 4.7 kWh an hour for September's last days with the month at 8.97 kW, for 0 NOK. Affects HLD INV-10; D2 §5.4, §5.11, §9 43, §11; D8 §5.5.
**Rejected:** holding the chosen step - buys 0, costs EV room and comfort. Advice alone - the house still sheds for nothing. Relaxing to the full slack instead of the reached step - inside a `StepTable` step the metric's rise is free, and `auto` already has the rule and its tests.

### D-0691 · A mode floor's reversal waits out its dwell

`ModeCfg` takes `min_on_s`/`min_off_s`, floor heating passes its answers (900 s each by default), and D4 §5.10's row 7 gates the comfort ↔ shed reversal, `urgent` and `blunt` excepted as for every kind. `ModeKind.dwell_s()` returned `(0, 0)` though D4 §6.1 asks the household for both: the reference bathrooms went heat → eco → heat in 10 min 5 s and 10 min 6 s. Affects D4 §5.5, §5.10, §6.1, §9 48, §11.
**Rejected:** hiding the fields for the mode kind - for a floor thermostat the eco flip is the relay decision. Raising the mode's command interval - it limits writes both ways; the dwell is the reversal clock.

### D-0692 · The fixed-price saving's name, and a partial month says so

`SiteFixedPriceSavingsSensor` sets `_attr_translation_key = "site_fixed_price_savings"` after `PowerplanEntity.__init__`, which set the bare key and so matched no translation: HA named it by device class, `sensor.<site>_monetary_balance`. A registry migration renames an entity id still carrying the fallback, once, and a surface test reads every entity's effective key off the instance. `sensor.<site>_cost` carries `partial` and `energy_since`, the month card says "Siden {date} …" (D12 R5), and a load's savings compare `settled_cost` with `counterfactual_cost` - the live cost includes slots the counterfactual doesn't cover yet (bathroom 1: 1.49 against 0.74 NOK). Affects D8 §5.5, §5.16, §9 46, 47; D12 §5.19, §9 33.
**Rejected:** prorating the capacity fee in a partial month - the fee is billed for the whole month (D11 §2), and a prorated number is one no bill shows.

### D-0693 · An answered role binds when its entity is ready; the startup release stays
