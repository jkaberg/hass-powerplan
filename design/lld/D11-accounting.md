...
D7's close carries the site's production (`__site_production__`, a slot integral next to import and export, only for a site that measures it) and per load the surplus kWh its adopted plan meant to take in the slot (`PlanSlot.surplus_w` over the overlap, `inf` for a load whose `Demand.import_w` is 0, a battery). `close.py::_attributed` gives each claimant `min(claim, drawn)` from the pool `production − export`, pro rata to the kWh drawn when the pool is short. Without a production reading the pool is `export − import + Σ loads`, the surplus left after the house, and the load's slot is `ESTIMATED`. A load that discharged in a slot the site exported net has its whole slot at the export price. `PricedSlot` carries `sun_kwh` and `sun_price` (0 without an export curve), and `entry_cost` prices the two parts. The counterfactual house's surplus is `sun_cap_kwh = max(0, export − import + Σ loads)`, the same panels less the uncontrolled draw, and needs no production sensor. At settlement `cf_sun_kwh = min(cf_kwh, cap)`, or the slot's own `sun_kwh` where the counterfactual is the actual energy, so an own slot still saves exactly zero. Re-pricing only moves the grid parts. The site's figures are untouched and `cf_energy_cost` accrues the same per-slot differences, so §9 10's identity holds by construction (§9 20, D-0656).
