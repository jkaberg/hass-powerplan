...
| `energy` (ev) | `EnergyStore` | charge at `max_w` from the plug-in slot until `required_kwh` is delivered | plug-in = `Demand.wants` rising edge with `required_kwh`; `pending = required_kwh` (**WP0.10a**: *not* `/ charge_eff` - `EnergyStore.required_kwh` already returns energy at the wall, so dividing again charges the losses twice, and §9 5's own 19:44 only comes out without it; D-0174) **plus the slot's own `measured_kwh`** (**WP0.10**: the demand is read at the slot's close, after whatever the slot already delivered, so what the car asked for *at the edge* is the two together; D-0269); each slot `kwh = min(pending, max_w·dt)`, and the slot in which `wants` falls still charges what was pending, bounded by the slot - the car charged until it finished, and so does the shadow (D-0269). A **later** rising edge re-latches `max(0, asked − latched + real)`: what the car asks now, less what it asked at the last edge, plus what it really drew in between (`ShadowState.latched_kwh`, `real_kwh`). A BLE link that dropped and came back - the EV type wants nothing while offline, D4 §6.2 - asks exactly what is left and re-latches nothing; a car back from a drive asks for the drive, even when powerplan never topped it up in between (**WP0.10**, D-0269: on the reference house the first rule alone charged the weekend's 22 kWh backlog three times a day). `required_kwh` unknown (no SoC): the session's slots are **deferred** - listed in `deferred`, the load's figures carry `pending = True` - and at session end (wants falls) the shadow is stepped from plug-in at `max_w` for the session's actual kWh and each slot priced once (§2); a session still open after 7 days is closed with what is known. Force: the real load charges now and so does the shadow - no savings, correctly |
...
**In code (D-0304).** The `cycle` row's shadow is `on_request.py::OnRequestShadow`: `ctx.demand.wants` rising is the request edge (`ShadowState.run_started_at` anchors there, kept - not cleared - once the profile's own `duration_s` has elapsed, so a real request still `wants` does not read as a new one on the next slot); `ctx.params.cycle_profile` (a new `LoadParams` field, populated once at load-add time from the type's own *default* profile, `appliance_cycle.py::profile_of`, never a live learned one - no shadow chases one yet, for any kind) supplies the duration, the ten-segment shape and the total energy; each slot's kWh is the same segment-overlap the real plan's own `_energy_by_slot` computes for its chosen start (D5 §5.6), applied to the elapsed window since the request instead. `demand.wants` turning false - the run finishing or being cancelled - is the one signal that ends the shadow's run; D11 §9 7's own wording ("a cancelled request cancels the shadow run") needs no separate case. The `tank` row (§9 6) is not built - `ShadowCtx.draw_off_kwh`/`.legionella_active` are still dead fields nothing populates, and a tank shadow that ignored real household draw-off would be a systematically wrong number rather than an absent one (`design/PLAN.md`'s WP3.3 row).
