...
**Circuit sub-meters.** A circuit (D6 constraint) may bind a `MeterSource` of its own - typically power and per-phase currents only. It is a plain second `MeterSource`; no `WindowMeter` is built for it (circuit limits are instantaneous). Without a sub-meter, circuit power is the sum of its members' measured power (with the settling rule, §5.8) plus a configurable `unmetered_w` allowance.
