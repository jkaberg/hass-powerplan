...
1. Only what the company's own public page loads for any visitor: no login, no key, nothing a browser wouldn't fetch.
2. A User-Agent naming the integration and its repository; one listing call when the flow opens, one document call per renewal; responses cached in memory for the flow's life.
...
4. Every adapter has a captured fixture and a contract test (the fields it reads, which paths), and a nightly canary (§5.7) that tells the maintainer when the page changes - the household only sees "not supported" and the next tier.
5. The site's terms of use are read and recorded per adapter (O14).
6. **Downloads are transient.** A response lives in the memory of the one flow or renewal that fetched it (`Http`'s cache) - no file, no temporary directory, no extraction: an archive is read member by member. The flow releases it once the copy is taken or the flow ends, a renewal when it returns, and the copy in `entry.data` is the only thing kept. `tests/providers/tariffs/test_fri_nettleie_fetcher.py` checks that `providers/tariffs/` writes no file.
...
| US | OpenEI URDB utilities, by the household's ZIP code (`address=`, D-0576) | EIA utility id |
| AU | CDR register (brands) + the distributor on the plan; a brand's plans filtered by the postcode once chosen (D-0576) | brand, distributor |
| BE (Wallonia, Brussels) | CompaCWaPE and BruSim `postal_codes?code=` → the entry's grid company (D-0575) | the platform's postal-code id |
| FI | sahkonhinta.fi `getdsocollection` → the postcode's grid companies, naming the household's own table (D-0576) | company name |
| NO (postcode) | Kartverket's open address API: `ws.geonorge.no/adresser/v1/sok?postnummer=` → municipality number, `kommuneinfo/v1/kommuner/{nr}` → county (7010 → Trondheim 5001 → Trøndelag) - official, no key; the municipality settles the VAT zone and the tiltakssone, and NVE maps it to the grid company | municipality number |
...
**Norway's source: fri-nettleie, fetched from GitHub, for every company.** It's the data every Norwegian aggregator is built from, complete where Strømpriseridag is lossy (the rules by hour, `virkedag`/`helg`/holidays and months; every version with `gyldig_fra`/`gyldig_til`; household, cabin and small-business groups; the operator's own page per file), and actively maintained - an automated job, over a dozen contributors, most files updated within the year (each carries `sist_oppdatert`). The national overview itself counts "a data file on a known format and location" as an API, so it ranks as **T1b** (country-wide, third-party), fetched as one tarball when the flow opens (405 kB, 0.5 s). The `fri_nettleie` adapter reads its basis as published (excluding VAT and levies, no correction needed), credits it (CC BY 4.0), and guards staleness: a company whose file has no version valid today, or whose `sist_oppdatert` is older than twelve months, is shown with that date and its tariff confirmed by the household (rule 8). Strømpriseridag (the same data, lossy, mislabelled) and the per-company APIs (Digin at Elvia and Glitre, Tensio's page) are **nightly cross-checks only**, and no household key is needed (O13). The canary checks fri-nettleie's contract - every company in the archive gives a copy. Strømpriseridag names fri-nettleie as its source, so it's no second opinion; Tensio's `robots.txt` signals `ai-input=no`, so its adapter waits; Digin needs the maintainer's keys as CI secrets (D-0564). The archive is read member by member: `tariffer/*.yml` (74 files, 199 household tariffs) and Elhub's `grid_owners.json` (GLN → organisation number, to find the company in NVE's list); `tariffer/old/` is skipped (D-0560).
...
|---|---|---|---|---|
| SE | T1a Eltariff catalogue + standard (9 companies incl. E.ON, Göteborg Energi, Kraftringen, Tekniska verken); for the rest, **Ei's household file** (T6, D-0569) - the regulator's Excel at a stable URL, yearly: 137 companies, 493 products by company and network area, authority fees, fixed fee per fuse (16, 20, 25 A) and energy fee - one rate, or two without their hours - excl. VAT; **no power fee** (Ellevio's household power charge is absent) | other companies' price pages, to survey for the power-fee companies; Elprisguiden (all 119 companies) is excluded, its `robots.txt` disallows `/api/` (§5.2 rule 3) | - | the Eltariff companies whole; a company without a power fee whole from Ei's file (a two-rate energy fee asks its hours); a power-fee company outside Eltariff: its page (T4) or the rule template |
| DK | T1a Energi Data Service (31 charge owners) | - | - | all |
| BE | Flanders: T1a Fluvius open data (areas only); Wallonia and Brussels: **T1a** CompaCWaPE and BruSim (§5.9), postcode → grid company → rates by meter type | - (V-test has no JSON, §5.9; Fluvius's pages load no tariff API) | Flanders: VREG XLSX (T6, sign-off); VREG is now the Vlaamse Nutsregulator, its XLSX and per-area PDFs on `vlaamsenutsregulator.be` | Wallonia and Brussels all; Flanders areas by API, rates from VREG's XLSX |
| US | T1a OpenEI URDB | - | - | ≈ 3 700 utilities |
| AU | T1a CDR (some brands geo-restricted) | - | - | retail plans by brand |
| FI | none found: Energiavirasto's `Tilasto-Viimeisimmät-Siirtohinnat.xlsx` only holds average prices per type customer, taxes included, and is years old; sahkonhinta.fi's API has the directory, not transfer prices; porssisahkonhinta.net disallows `/api/` (§5.2 rule 3); vertaaensin.fi is prose | DSO price pages, to survey, the 15 largest first (≈ 70 % of users; Caruna, Elenia, Helen > 40 %) | - | none: the directory by postcode, then `custom` |
...
| AT | per network area, set by E-Control's regulation (SNE-VO); **households billed on power from 2027** (≈ 30 % power / 70 % energy at the start) | `PeakTariff` (15-min) from 2027 | E-Control's Tarifkalkulator is behind a captcha (Friendly Captcha), excluded by §5.2's conduct; the SNE-VO's per-area tables (T6, sign-off) until E-Control publishes data | T6 | read |
| CH | per municipality and operator (≈ 600), ElCom-supervised; HT/NT | grid TOU; `NoPeak` | ElCom `strompreis.elcom.admin.ch/api/graphql` (live, the site's own backend) and LINDAS linked open data (SPARQL), CSV | **T1a** | built (D-0602) |
...
| PL | per DSO (5 major), URE-approved: G11/G12/G12w/G13 zones, G14 dynamic; fixed fee per phase, subscription, quality and transition fees | grid TOU per DSO; `NoPeak` | **Tauron Dystrybucja**: its calculator page (`taniej.tauron-dystrybucja.pl`) embeds the whole rate card as JSON (`electricCalculatorConfig.stawki`: fixed 1- and 3-phase, variable per zone for G11, G12, G12w, G13, G13s by season and day, G14d's four zones, subscription, quality, transition), incl. VAT (its `VAT` row); the zones' hours are the tariff's rule; OZE and KOG are the PL module's levies; G11, G12, G12w offered. The other four DSOs (PGE, Enea, Energa, Stoen): tariff PDFs; URE's `maszwybor` publishes supplier offers as Excel; taryfypradu.pl (a private calculator for all five) answers from an AppSync backend behind an API key, excluded by §5.2 rule 1 unless its owner agrees | T4 (Tauron) / T6 | built (D-0606) |
| CZ | ERÚ national prices (T6); breaker-size fixed fee; **HDO**: the grid switches low tariff per HDO code | grid TOU and a switched window from HDO; `NoPeak` | ČEZ Distribuce's anonymous portal service (`dip.cezdistribuce.cz/…/casy-spinani?path=switch-times/signals`, a POST with the household's EAN, meter serial or place; per-signal times by date, as the `cez-distribution-hdo` client and three HA integrations read it) now asks a captcha (`CPT-002`), so it's excluded by §5.2 (D-0605); EG.D, PREdi to read; the EAN only ever goes to the household's own grid company | T4 (excluded) | excluded |
| SK | ÚRSO; dual tariff, tariff programme switching times per DSO | as CZ | ZSDIS: every HDO code's switching times (32 household, 12 business codes, weekday and weekend) sit as a JSON literal in its public page (`household_rates`); the household picks the code on its meter's label. ZSDIS's REST APIs (HDO code per EIC) are for suppliers with a distribution contract. SSD: an old XLS only; VSD to read | T4 (ZSDIS) / T6 | built (D-0604) |
| HU | MEKH system usage fees (transmission 3.39 Ft/kWh); H-tarifa (heat pumps, separate meter), controlled tariffs | per-load tariff; `NoPeak` | MEKH's decision (H 2995/2025) and decree 4/2026 in the national legal database; the DSOs' fee tables (MVM Hálózat) | T6 | read; no API found |
| SI | **5 time blocks** (seasons, Nov–Feb high), an **agreed power per block** per metering point set by the DSO, excess power charged | `ContractedPower` per block, `on_exceed = surcharge` | the national methodology (AGEN) as the rule; the household's own agreed powers and 15-min readings from **Moj elektro's documented OpenAPI** (`api.informatika.si/mojelektro/v1`, the household's own token; a community HA integration uses it) | rule + T3 | read |
| HR | HEP ODS: Plavi (single rate 0.037608 €/kWh), Bijeli (VT 0.044446 / NT 0.020514 €/kWh) | grid TOU; `NoPeak` | HEP ODS tariff sheet | T6 | read (secondary) |
| RO | ANRE distribution tariff per distribution zone (8), yearly; transport, system service, cogeneration, green certificates, excise | `NoPeak`; the state party's levies | **ANRE's offer comparator** (posf.ro, the regulator's consumer site): `comparator/api/index.php?request=comparator-electric`, open, no key, no captcha, returns every offer with the zone's `tarif_serviciu_distributie`, `tarif_transport_tl`, `tarif_serviciu_sistem`, `taxa_cogenerare_inalta_eficienta`, `contravaloare_certificate_verzi`, `acciza` and `tva` (lei/kWh; eg low voltage: zone 6 0.33325, zones 1, 7, 8 0.31739, zone 4 0.38791); `get-judete` maps county → zone. One call per renewal, the answer is ≈ 1 MB. Distribution, transport and system service are the grid copy; cogeneration, green certificates and excise the RO module's | **T1a** | built (D-0603) |
| GR | regulated network charges; DEDDIE dual-zone reduced rates (winter night 02–05, midday 12–15) | grid TOU | DEDDIE's published winter/summer schedule and the regulated charges, in the GR module (O22); its dual-zone checker app is behind reCAPTCHA and Incapsula, excluded (§5.2) | rule | read |
| ES | **national** regulated tolls and charges (2.0TD): contracted kW for P1 and P2, energy P1/P2/P3 by hour, weekends and national holidays P3, the same for every distributor | `ContractedPower` (P1, P2) + grid TOU | the energy term per hour from REE's open PVPC file (`api.esios.ree.es/archives/70/download_json?date=`, no token): `TEUPCB` €/MWh, eg P1 97.55, P2 29.27, P3 3.29, in 2.0TD's windows; the power terms (€/kW·year) from the yearly CNMC circular and Ministry order, in the ES module as national law (O22) | T1a (energy) + rule | built |
...
| LU | Creos (single DSO): **reference power** categories assigned from history, a **per-kWh surcharge** on energy drawn above it | new term: overrun energy above a power threshold | ILR's decision and Creos's guide (T6): categories 3, 7, 12, 17, 27, 43, 70, 100, 150, 200 kW; each kWh above the reference power in a **15-min** mean pays the surcharge on top of the volumetric fee; a night surcharge 22–06 for storage heating. The household's own Pref is on its bill, on my.creos.net and on **Leneda**, the national data platform, whose API takes the household's own key (two HA integrations use it). Pref is re-assigned monthly from the last 12 months | T6 + T3 (Leneda) | read; tariff model gap (O20) |
...
| LV | Sadales tīkls: capacity maintenance fee by connection (1-phase 1.26, 3-phase 3.50 € excl. VAT/month) + energy | `NoPeak` (fixed fee) | Sadales tīkls | T6 | read (secondary) |
| LT | ESO: one, two or four time zones; "Efektyvus" plan with a fee per kW of permitted power | grid TOU; fixed per kW | ESO's page: an HTML table, no JSON behind it (Standartinis, Efektyvus with 1.00 €/kW/month of permitted power, Namai with 3.00 €/month; one, two or four zones; day 07–23 weekdays winter, 08–24 summer) | T6 | read |
...
| IS | Veitur, RARIK and others: distribution per kWh (Veitur 11.52 kr before VAT), urban/rural areas; no household capacity | `NoPeak` | the operators' price lists | T6 | read (secondary) |
...
| `elpris_dk` | DK | elpris.dk's `static.json`, `distributionAreaCharge_{area}.json`, `nationalCharges.json`; Datahub for later seasons (D-0571) and where elpris.dk has no usable `flex` hours (D-0682) | the 34 grid areas of `static.json` | the area charge's `flex` records in force (`validFrom`/`validTo`, 24 hourly amounts) + Energinet's per-kWh tariffs → the grid's energy charge; subscriptions per month → the fee; `NoPeak` |
| `ei_household` | SE | Ei's `Hushållskunder.xlsx` (T6, D-0569) | companies with figures for the year; products are network area × customer group | one version per year; fees per year (authority + fixed); one rate, or two with the first rate's hours asked; power fee asked; `NoPeak` |
...
| `vreg_xlsx` | BE (Flanders) | the Vlaamse Nutsregulator's yearly `Distributienettarieven elektriciteit <year>.xlsx`, read with the standard library (zip + XML) | the eight Fluvius areas, the overview sheet's columns | one version per year (new year); `Gemiddelde maandpiek` EUR/kW/year excl. VAT per area; the rule - 15-min windows, the month's highest, the mean of the last 12 months, `min_kw` 2.5 - is the regulator's and the parser's, cross-checked each fetch against the sheet's own "minimale bijdrage" (= 2.5 kW × the rate) |
...
|---|---|---|---|---|
| NO | 25 %; 0 in Nordland, Troms, Finnmark | forbruksavgift per kWh excl. VAT by its dated periods, 0 in the tiltakssone; Enova 1.0 øre | the postcode's municipality (step 0), else NVE per DSO; the tiltakssone municipality list | Skatteetaten (no API) |
| SE | 25 % | energiskatt (0.36 SEK excl. VAT), reduced in listed municipalities | asked in step 1b for every Swedish company (D-0568) | Skatteverket (no API) |
| DK | 25 % | elafgift by its dated periods, in the DK module (D-0572); Energinet's system and transmission tariffs are the copy's | national | the module: skat.dk; the copy: elpris.dk `nationalCharges.json` (T1a) |
...
| D2 tariff model | a source needs a `day` price unit and a power factor (AU), nth-highest (FI Helen); seasonal prices inside a version (URDB) are dated versions | `day` and power factor in TS.5; nth-highest (D2 §10) |
...
| D2 holidays | an operator's "helligdager" may not be the national calendar | the source's own definition where it states one, else national, noted in the summary; checked per operator in TS.3 |
...
| D8 flow and surface | §6's order and words; the action `powerplan.refresh_tariff` (D8 §5.7, with `services.yaml` and translations); repairs `tariff_stale`, `tariff_review`, `tariff_source_unreachable`; diagnostics carry the copy's provenance and next renewal (no personal data) | TS.2 |
...
| D11 accounting | savings by party; the counterfactual on the same copy and taxes (INV-69); bills as the household pays them | TS.1 |
| D12 dashboard | the price stack by party; the grid's day/night bands on the timeline; the party in every "why" | TS.2 |
| export (plusskunde) | grid feed-in terms (Tensio: negative energy charge = spot × marginal-loss rate, 6 % winter / 4 % summer); export pays no VAT or levies | `GridTariff.feed_in` where published (fri-nettleie doesn't; Tensio's PDF does), Phase 7 |
...
| location | a coordinate isn't an answer; a postcode is, and every regulator's consumer site keys on it (F14) | step 0 (O17): the grid company, the municipality (tax zone), the price area (TS.2) |
| moving / changing grid company | a new operator is a reconfigure | history continues on windows, the level on the new rule (TS.2) |
| multi-site | each site its own entry and copy | no change |
| VAT over time | rates change (CY, GB, ES's temporary reductions) | dated rates in the module; each slot at its date's rate; `vat_check.py` warns on TEDB drift (TS.1) |
...
| NO cabins and the Nord-Norge exemption | mval. § 6-6 exempts "husholdningsbruk"; [mval-forskriften § 6-6-1](https://lovdata.no/forskrift/2009-12-15-1540/§6-6-1) a) counts "fritidsboliger, herunder hytter, setrer, koier o.l." as household use | a cabin in Finnmark, Troms or Nordland is exempt like a home; the `nord` zone holds for either product (D-0565) |
...
| licences and attribution | fri-nettleie and Strømpriseridag are CC BY 4.0 ("fri bruk med lenke"); URDB and CDR have terms of use | every source declares `attribution`; the flow credits the country's sources under the grid company step, in the summary, diagnostics and docs (§6.1); terms read per source |
| network, privacy, security | outbound calls to GitHub, NVE, Kartverket, Eltariff hosts, Energinet, the regulators' comparators, OpenEI, CDR hosts; the postcode only to an official directory (O17); a meter id (HDO) only to the household's own grid company; untrusted input | TLS, timeouts, size caps (≤ 5 MB), `yaml.safe_load`, every parsed tariff through the loader (D2 §2); downloads in memory only, released after the copy (§5.2 rule 6) |
| source failure or abandonment | a community dataset can stop | the copy keeps working; `tariff_stale` after its last version; the template path always exists; `preset_age.py` extended to the country modules' dated facts |
...
| backtest | `tools/backtest.py` reads a tariff file | takes a stored copy or a fixture (TS.6) |
| user docs | one page per party: "Nettleie", "Strømavtale", "Avgifter" | D14 |
...
| file(s) | what it is | fate | WP |
|---|---|---|---|
| `presets/no/tensio-ts.json`, `no/tensio-tn.json`, `no/elvia.json` | a company's prices | kept only as the offline migration source for entries without a copy (§10), then **removed** (INV-70); their tables become the TS.3 fixtures' expectations | TS.6: `tests/fixtures/presets/no/`; an entry still on one migrates to a copy fetched at the first renewal (D-0600) |
| `presets/be/fluvius-<area>.json` (8) | a company's prices from VREG's sheets | **removed**; the eight rates become the `vreg` XLSX adapter's fixture expectations | TS.6 (D-0600) |
| `presets/se/ellevio.json` | a company's fact (no capacity) | **removed**; Ellevio (not in Eltariff) falls to the SE rule template (`no_peak`) | TS.6 (D-0600) |
...
| `tests/fixtures/presets/no/tensio-ts-2027.json` | the benchmark's synthetic next-year version | **kept** as a rule-format fixture next to the company files, read through `tests/builders/presets.py` (D-0601) | TS.6 |
| `tests/fixtures/tariff_sources/*` | fri-nettleie captures, hand-read tables | **kept** | TS.3 |

### 12.2 Code

| where | before | fate | WP |
|---|---|---|---|
...
| `entry.data.tariff.spec` (D-0520) | preset JSON + the household's kW | migrated to `HouseholdPrice` at start (§10) | TS.1 |
...
| add-on steps `modifier_tou_schedule`, `modifier_levy`, `modifier_vat` | the household types the grid's energy charge, levies and VAT | `tou_schedule` leaves the supplier step (it only stays as "the supplier's own time-of-use offer"); `levy`, `vat` become party 3's overrides (O4) | TS.2 |
...
| `core/pricing/modifiers/tou_urdb.py` (paste a URDB rate) | a US household pastes JSON | becomes the `openei_urdb` adapter's energy half; the paste option is removed (D-0578) | TS.5 |
| steps `tariff`, `tariff_preset`, `tariff_steps`, `tariff_limits`, `tariff_bills`, `tariff_target` | the grid company, its table, the template's numbers, the target | kept and extended to §6's steps 1–1e (`tariff_steps`/`tariff_limits` for templates; `tariff_bills` for rolling periods) | TS.2 |
...
| `tools/preset_age.py` + CI step | warns on shipped versions verified > 6 months ago | covers rules and the country modules' dated facts (no prices left to age); `tools/vat_check.py` next to it | TS.1, TS.6 |
| the country field (`flow/steps.py`, `CountrySelector(CountrySelectorConfig())`, only asked without `hass.config.country`) | full names, no pre-selection | kept; pre-selected from the time zone the country modules declare (0′) | TS.2 |
| `sources/base.py`: `merge`, `next_check`, `Question` | renewal and gaps for fetched copies | reused by the renewal (§10) and the confirm step (1c); `RENEW_AFTER_DAYS` = one month | TS.2 |
| `tools/backtest.py --preset` | loads a shipped file | `--tariff` takes a stored copy or a fixture (D-0600) | TS.6 |
| `tests/builders/houses.py` (`tensio()`, `be_quarter`, `nl_pv`) | shipped files and the next-year fixture | fixtures only (D-0601) | TS.6 |
...
| WP | produces | exit |
|---|---|---|
| TS.1 Model, composition, first cleanup | `HouseholdPrice` by party, `Basis`, the country registry and one module per country with its VAT (dated, regional), levies and tax zones (§5.1, §9.1, O3, O21), `tools/vat_check.py` against TEDB, the chain by party (§8) with the state stage by the slot's date (§18 G16), D2 `spec()`, fixed fees per day (G21); price-source basis (O5); rules moved to `core/tariffs/rules/`; older copies and add-ons migrated (§10); `_preset_components`/`_preset_modifiers` and `includes` removed (§12); D11 savings by party | one Tensio TS month billed and composed identically from excl-VAT and incl-VAT copies; no component counted twice across every D1 source × basis; a Norgespris house still moves the EV to the grid's night; the reference house migrates offline |
| TS.2 Source framework, flow by party, renewal | the ladder and registry (§5.1), each country module's source ladder and time zones (0′), price fields labelled incl. VAT and the VAT dialog only without a country module (1c′) with each source's `attribution` and the credit note (§6.1), the monthly renewal timer and the `refresh_tariff` action (§10), directories (§5.3), the postcode step and its resolvers (O17), the nightly canary (§5.7), §6's steps and words (en, nb), reasons by party (§7), renewal and repairs (§10), `_RECOMMENDED` and the grid/state add-ons removed from the supplier step (§12); D12's price stack by party | the three-party walk in both languages; `refresh_tariff` appends a new version, replaces a corrected one, keeps the rest and answers what it did, and leaves the copy untouched when the source fails; the timer fires one month after the last fetch or seven days before the last version ends, whichever is first; the credit note under the grid company step for every country with a source, and a registry test that no licence-requiring source lacks its credit; no screen offers an earlier party's component (INV-74); no HTTP at setup; a fake source at each tier falls to the next |
| TS.3 Norway | `fri_nettleie` (T1b) with its staleness guard and every method (G6 weekly groups, G7 exclusive thresholds, G8 `MND_MAX`, `OV_TREFASE`), NVE and Kartverket zones; the nightly check of fri-nettleie's contract (D-0564); customer group; downloads held in memory and released (§5.2 rule 6) | the 12 hand-read tables reproduced; all 199 household tariffs parse, none refused; Elvia's 17 May priced at the holiday rate; a stale file shown with its date and confirmed; Tensio TN asks the county, Elvia doesn't |
| TS.4 Sweden, Denmark | `eltariff` (T1a), `ei_household` (T6, D-0569), `elpris_dk` (T1a: grid areas and national taxes, with Datahub's next season, D-0571) with `datahub_pricelist` (T1a) as its cross-check; G22 (D-0570) | Göteborg's seasonal peak tariff; Radius's winter table |
| TS.5 Belgium, US, Australia | Fluvius areas (T1a) and VREG's XLSX for their rates (T6, O15, D-0577); one adapter for the CWaPE/Brugel platform (T1a, D-0575: Wallonia's 13 grid companies and Sibelga, by postcode and meter type; excise and the energy contribution to the BE module); sahkonhinta.fi's directory for FI (postcode → grid company); `openei_urdb` (D-0578), `cdr_energy` (D-0579); D2's `day` unit (G9), power factor (G10), `nth` (G5) and several peak charges per version (G4, D-0573); `tou_urdb` paste removed | VREG rates equal the eight PDFs; APS summer and winter; a CDR plan with its confirmations. Fluvius's municipality directory and Wallonia's Impact meter open |
| TS.6 Retire shipped prices | §12's removals; backtest and benchmark houses on fixtures; `preset_age.py` on rules and tax data (D-0600, D-0601) | the house on its fetched copy; no company price in the repository (INV-70) |
| TS.7 Europe | per-load grid tariffs (G13) and grid-switched windows (G14, G15); priced soft limits (G1, O23) and LU's `energy_surcharge` (G2); standard-time filters (G11); Tempo's colour × hour (G12); PT's VAT bands (G17); SI's excess (G3), IT's tolerances (G20), PL G14dynamic (G19) read first; Switzerland (ElCom, T1a), Slovakia (ZSDIS HDO codes, T4), Romania (ANRE's comparator, T1a), Poland (Tauron's rate card, T4) and Spain (ESIOS) adapters, Czechia excluded (captcha, D-0605); Austria on the SNE-VO tables (T6, O15) until E-Control publishes data; national rule templates for IT, PT, IE, FR (with Tempo events); SI per-block contracted power; LU's `energy_surcharge` (D2, O20); DE Modul 3 after reading GET AG's terms | a §14a Modul 3 heat pump billed on its own curve next to the house; an LU house charging 11 kW over a 7 kW reference power at the surcharge, not stopped; an HDO water heater that never runs outside its window; ElCom's tariff for one municipality reproduced |

## 18. Can the engine say every tariff?

Every variation this survey found (§5, §9, §5.10) and every household tariff in fri-nettleie (199; `tariffer/old/` - three companies merged away - not counted), checked against the code: D2's tariff model and evaluator (`core/tariffs/model.py`, `evaluator.py`, `contracted.py`), D1's modifiers (`core/pricing/modifiers/`), and D6's ladder (`core/allocation/ladder.py`).

### 18.1 Already said

| variation | who | how |
|---|---|---|
| mean of the month's top-N daily maxima, distinct days, steps | NO (186 of 199 household tariffs: `TRE_DØGNMAX_MND`) | `PeakTariff(per_day="max", per_period="mean_top_n", n=3)` + `StepTable` |
...
| # | variation | who | before | change | where | WP | size |
|---|---|---|---|---|---|---|---|
| G1 | a limit whose excess is **priced**, not tripped | SI (agreed power per block), LU (reference power) | `contracted.py` turns every `ContractedPower` into a `HardLimit`, `on_exceed="surcharge"` included, and D6's ladder enforces it as rung 1: LU's 7 kW would stop an 11 kW charge that costs 0.0765 €/kWh extra | `trip` stays a hard limit; `surcharge` and `energy_surcharge` become a **priced soft ceiling** - a marginal cost in the plan, like a capacity step (O23) | D2 §5.8, D6 | TS.7 | M |
...
| G4 | two peak charges at once (all-hours demand + on-peak demand) | US demand rates | `TariffVersion.peak` returns the first `PeakTariff` | `peaks: tuple[PeakTariff, ...]`; the evaluator bills each; the ceiling is the lowest (D-0573) | D2 §3, §5 | TS.5 | L |
| G5 | the n-th highest, not a mean | FI Helen | `per_period` is `max` or `mean_top_n` | `per_period="nth"` (D-0573) | D2 | TS.5 | S |
...
| G7 | a threshold that isn't inclusive upward | NO (`terskel_inkludert: false`, 11 of 199) | `StepTable` is always inclusive upward | `StepTable.inclusive: bool` | D2 §5.3 | TS.3 | S |
| G8 | fri-nettleie's other methods | NO (`MND_MAX`, `OV_TREFASE`) | only `TRE_DØGNMAX_MND` parsed | parser, not engine | TS.3 adapter | TS.3 | S |
| G9 | a demand price per day | AU | `price_period_unit` is month or year | `day` (D-0573) | D2 | TS.5 | S |
| G10 | demand in kVA | AU | kW measured | a fixed power factor per site, asked (D-0573) | D2, D3 | TS.5 | S |
...
| G12 | a day type × time-of-use matrix; a day that starts at 06:00 | FR Tempo (3 colours × HP/HC = 6 prices, day 06–06) | one price or multiplier per colour, day at midnight | `DayTypeRate` gets periods; `day_starts_min` | D1 §5.4 | TS.7 | S–M |
...
| G15 | a controlled circuit whose times aren't published | HU (vezérelt) | - | not plannable: accounted only, said so | D4, D11 | TS.7 | S |
| G16 | VAT and levies that change on a date | GB's temporary zero rate, CY's ending reduced rate, every new-year levy | `vat.rate`, `levy.amount` are static | the state stage reads the country module's value **for the slot's date** | D1 §5.4, §8 | TS.1 | S |
...
| G18 | net metering: export valued at the import price up to the period's import | US NEM, BE-WAL's old compensation, NL until its end | `export_price` is fixed / spot − / spot × | `export_price(mode="net_metering")` | D1, D11 | Phase 7 | M |
| G19 | a grid zone schedule published day-ahead | PL G14dynamic | - | read first; if day-ahead, an event-fed `tou_schedule` like `day_type` (D-0611) | D1 | TS.7 | ? |
| G20 | two tolerance bands on a trip limit (a margin for good, a larger one for a while) | IT | one `tolerance_pct` × `tolerance_s` | a tuple of bands; ARERA states one (D-0611) | D2 | TS.7 | S |
| G21 | a fixed fee per day | PT, AU | fees per month or year | `FeeVersion` per day (bill only) | D13 §3 | TS.1 | S |
| G22 | a grid energy charge that's a share of the hour's spot price (loss compensation) | SE Kraftringen, Skånska Energi (5 %), E.ON capacity tariffs (1.6 %) | the grid's energy charge is fixed per period | `EnergyVersion.spot_share` (D-0570) | D1 §5.3, D13 §3 | TS.4 | S |
...
**Verdict.** The tariff model says the capacity rules of every country that bills them - 186 of 199 Norwegian household tariffs as they are, 12 of the other 13 with G6–G8 and Tinfos's `UKJENT` asked (§5.6) - and D1 says every energy-charge shape surveyed except the four marked above (G11, G12, G17, G19). What it can't say is **structural in two places**: a load with its own tariff (G13) and a limit that's priced rather than tripped (G1). Both are TS.7's and in v1 (O24), and G1 is decided (O23: a priced soft limit, INV-36 narrowed to tripping limits). TS.1–TS.5 only need G4–G10, G16 and G21 - small additions.
