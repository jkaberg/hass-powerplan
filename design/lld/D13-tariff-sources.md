...
| `elpris_dk` | DK | elpris.dk's `static.json`, `distributionAreaCharge_{area}.json`, `nationalCharges.json`; Datahub for later seasons (D-0571) | the 34 grid areas of `static.json` | each tariff charge's `validFrom`/`validTo` and 24 hourly amounts + Energinet's per-kWh tariffs → the grid's energy charge; subscriptions per month → the fee; `NoPeak` |
