...
*(PLAN dec. 44)* **Pick entities by hand, built.** The flow's own device list gains a last entry, *Not on a device - choose the entities*. It opens one `EntitySelector` per role of the chosen type, filtered by the role's domain and unit. `DeviceView.from_entities` is matched against every profile (D4 §5.9) exactly as a device's view would be, so a row that matches by shape - `sungrow_modbus` over the mkaiser package - pre-binds what the household did not pick. A set no profile matches is refused with `no_profile`, naming the roles it lacked. The appliance's entities go on the fallback device (§5.16). It is open to any type, since `generic_number` and `generic_switch` match bare entities too. *(D-0676: one multi-entity picker, then the match step's per-role pickers; `no_profile` without the roles; the subentry's unique id is its first bound entity; the fallback device carries no `device_missing` for a load that never had a device.)*
...
| `notify_service_missing` | warning | no | the configured `notify` service does not exist (§8) |
| `prices_stale` (D-0580) | warning | no | no known price for the slot in progress for 30 min; the price refresher retries 1–2–5–10–15 min and clears it; not persistent (D12 §5.15 F12) |
| `load_error` | warning | no | a load raised in a tick and is held (D7 §8) |
| `battery_control_off` | warning | no | a battery row's command levers stay unavailable for 15 min, or a write is refused; the text names the row's prerequisite in the household's words (*Power Control Options* in the SolarEdge Modbus integration's options; *Inverter control via Modbus* on the Fronius; the Sigenergy remote EMS switch; the Tesla energy-command permission). The load's status says it is not steered (D4 §5.9). *(D-0672:)* raised by `RepairsWatch._battery_control_off` for a row with a `prerequisite`; its text names the row's `title` and the setting |
...
| the hard-limit step | gone: no site step writes `hard_limits`, `CONF_HARD_LIMITS` is deleted, and an entry from before this WP that still carries one is ignored, not migrated. The connection's own ceiling is D3's fuse and per-phase limit, already read by the engine | - |
...
**What changes.** A load's entities today live on a PowerPlan-owned device (`entity.py::load_device_info`, `DeviceInfo` with PowerPlan's own identifiers - the deprecated helper pattern, HLD §4). After this change they attach to the appliance's own hardware device (`Entity.device_entry`) wherever the load has one, cutting the entity count from 16–17 to 4–6 visible per appliance and removing the duplicate device from the integration page. Everything below is sourced against the installed HA 2026.9.3 (`.venv`/the cached wheel) and the `home-assistant/frontend` source; each claim from the spec is marked **verified** or **refuted** where it was checked.
...
Attributes: `next_start`/`until` from `Plan.slots` (the same data `sensor.<load>_plan_next` read); `granted_power` from `LoadStatus.grant.w`; `comfort_state` from `ComfortState` (`på mål`/`under mål`/`over mål`, ENT-25's already-planned three-state rule - "over mål" exists here because a `manual_override` can raise a setpoint above target, unlike ENT-25's original finding that core has no such field for the old `comfort_state` sensor); `reason` a translated sentence built the same way the load review's shadow sentence is (`flow/text.py`, D8 §5.15's R1). `reason` is the engine's English action reason (`LoadStatus.action_reason`). Two attributes sit beside it. `reason_key` is that reason as one of `ActionReason`'s closed set of keys (`core/loads/kinds/base.py`), or `None` before the first apply. `reason_params` holds the numbers the reason carries. Both are volatile, like `reason` (audit F-11), and are translated as a label under `state_attributes.reason_key` and as a sentence under `selector.action_reason` (D12 §5.6, D-0480, D-0481). *(D-0630)* While the state is `waiting` or `idle` and `next_run` is not empty, `reason`, `reason_key` and `reason_params` give the run ahead instead of the gate's last action: `planned` with `{time, kwh}` ("Planned from 22:00 · 7.6 kWh"), a key of `plan_status`'s own beside `ActionReason`'s set. Any other state keeps the gate's reason, which is what explains an appliance that is acting, held or overridden. `next_run` and `deadline_time` are local `HH:MM` strings of `next_start` (when it is after the snapshot; `""` while a run is in progress or nothing is planned) and of `deadline` (`""` without one), for tiles and badges, which show a datetime attribute as a full timestamp (D12 §5.6 v0.5, D-0489). States are mutually exclusive by construction (§5.2's precedence: `device_unavailable` checked first, then `manual_override`, then effective mode, then the plan/shed read) - one test enumerates every (mode, shed, override, health) combination the reference house's load types can reach and asserts exactly one state fires (D8 §9, new item).
...
31. *(§5.16, flagged open)* The gear dialog's device link: a manual check (not automated: the frontend's markdown-link navigation is not confirmed against source) that the relative `/config/devices/device/{id}` link in the reconfigure step's description opens the right device page from a running dev instance, in-app if HA's markdown renderer supports it, as a full navigation otherwise.
...
**Keep the deprecated `device_info` pattern a while longer, since our HA floor (2026.3.0) still runs it**. *For:* one fewer migration to write right now, and the floor line does not enforce the cutoff. *Against:* the reference house already runs 2026.9.3, past the pattern's stated cutoff (Core 2026.8) - code that already does not work in the house it is deployed to is not a deferral, it is broken; and every new load added between now and whenever this is eventually fixed would need a second migration. **Decision:** fix it now, in this change, since the spec exists precisely because it stopped working.
...
**Keep "pick entities by hand" in v1.x *(PLAN dec. 44)*.** *For:* the load flow deferred it until a profile could bind arbitrary entities, and a device-first flow is the one household-facing path the review checked (U.1–U.4). Pickers ask the household to know which entity is which. *Against:* the battery vocabulary is that profile. It matches by shape, so the pickers only confirm, and the fallback device is built. Sungrow - one of the largest hybrid-inverter brands, with no device-based integration of any size - is otherwise not steered at all. **Decision:** the entry is the device list's last line, and it builds only what a profile matches.
