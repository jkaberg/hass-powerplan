...
*(PLAN dec. 44)* **Pick entities by hand, built.** The flow's own device list gains a last entry, *Not on a device - choose the entities*. It opens one `EntitySelector` per role of the chosen type, filtered by the role's domain and unit. `DeviceView.from_entities` is matched against every profile (D4 §5.9) exactly as a device's view would be, so a row that matches by shape - `sungrow_modbus` over the mkaiser package - pre-binds what the household did not pick. A set no profile matches is refused with `no_profile`, naming the roles it lacked. The appliance's entities go on the fallback device (§5.16). It is open to any type, since `generic_number` and `generic_switch` match bare entities too.
