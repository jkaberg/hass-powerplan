...
**What changes.** A load's entities today live on a PowerPlan-owned device (`entity.py::load_device_info`, `DeviceInfo` with PowerPlan's own identifiers - the deprecated helper pattern, HLD §4). After this change they attach to the appliance's own hardware device (`Entity.device_entry`) wherever the load has one, cutting the entity count from 16–17 to 4–6 visible per appliance and removing the duplicate device from the integration page. Everything below is sourced against the installed HA 2026.8.0 (`.venv`/the cached wheel) and the `home-assistant/frontend` source; each claim from the spec is marked **verified** or **refuted** where it was checked.
...
**The naming/entity-id helper.** One function, `entity.py::appliance_entity_id(hass, device_id, key, platform) -> str | None`, called from each `LoadEntity.__init__` before `super().__init__()`, only for an entity whose device is a hardware device (never for a fallback-device entity, spec §3.1's "nothing to tell them apart from"):

```
translations = await async_get_translations(hass, hass.config.language, "entity", {DOMAIN})   # D-0410: same call flow/text.py already makes
name = translations[f"component.powerplan.entity.{platform}.{key}.name"]
name_without_prefix = strip_prefix(name)          # "PowerPlan-styring" → "styring"; "PowerPlan: lad til" → "lad til"; one regex, one test (en, nb)
device = dr.async_get(hass).async_get(device_id)
self.entity_id = async_generate_entity_id(f"{platform}.{{}}", f"{device.name_by_user or device.name} {name_without_prefix}", hass=hass)
```

D-0410's finding matters here: this call only *computes and preset-assigns* an object id (`internal_integration_suggested_object_id`); the registry's own `_async_get_full_entity_name` (unprefixed `suggested_object_id` path) is what actually decides the final id, with its own collision suffixing - so a translation lookup racing the registry's own is not a bug, both must agree by construction (the same translated name is what the entity's `name` also resolves to for display). The helper runs once, at entity construction, for a **new** entity only (an existing entity's `entity_id` is never touched - spec §3.1, INV-50); it is covered by one test per language against the reference house's device names (D8 §9, new item).
...
**Migration steps, each with its INV.**

| step | what | INV |
|---|---|---|
| 1 | Re-create every appliance sub-entry's entities with `device_entry` = the bound hardware device (`LOAD_DEVICE_ID`, already stored). `unique_id`s unchanged. | INV-50 (ids, history, statistics kept - verified above, no `device_id` column in the recorder schema) |
| 2 | Remove the old PowerPlan appliance device (`device_registry.async_remove_device`) once it has no entities left. | - (a device with no entities is inert; nothing else references it) |
| 3 | Remove the merged-away entities (`switch.<load>_force`, `number.<load>_force_max_hours` where replaced, `sensor.<load>_plan_next`, `_plan` (state half), `_comfort_state`, `_session`, `binary_sensor.<load>_shed`, `number.<load>_comfort_min_c`/`_max_c` where a device role now owns them, `time.<load>_deadline`/`_ready_by` → the merged `ready_by`) and raise **one** repair per site listing every removed id beside its replacement. | INV-50 (a merge repair is now allowed; kept ids keep their history) |
| 4 | Keep user customisations: a renamed entity keeps its name (HA's own registry behaviour - a `name` override on a `RegistryEntry` survives a `device_id`/`unique_id`-preserving update untouched); a disabled entity stays disabled (`disabled_by` untouched by this migration). | - (no invariant governs customisation preservation directly; it falls out of not touching `RegistryEntry.name`/`disabled_by`) |
| 5 | Strategy "Alltid på" → `control = Ikke styr` **and** the type's default strategy, except `generic_switch`'s on-call subtypes, which keep `control = auto`/`strategy = always` (D-0412). | HLD §3's `strategy = always` corollary (checked, unchanged - the exception exists so this corollary keeps holding for on-call loads) |
| 6 | Numeric priority → nearest of Lav (15) / Normal (30) / Høy (45), thresholds 22.5/37.5 (D-0411). | INV-66 (a derivation-table value materialising into an entity for the first time is still "materialised at setup", read here as "at upgrade") |
| 7 | The sub-entry's stored comfort value → written once to the device's own setpoint, only if it differs, then dropped from the sub-entry - as an ordinary WriteGate `SETPOINT` write scheduled in D7 §5.5's new step 5a, after release/restore, skipped entirely while the site is `off` (D-0417). | INV-26, INV-27 (the write is on-record from the moment it lands, so the very next observed change is classifiable) |
...
24. *(§5.16)* Naming and ids: for every appliance type of the reference house, in `en` and `nb`, a new entity's friendly name starts "PowerPlan" (hyphen before a noun, colon before a phrase) and its entity_id contains no "powerplan" and matches the translated-name-minus-prefix rule; an id collision with the hardware's own (`…_status` already taken) gets `_2`; an existing entity's id is untouched by the upgrade.
25. *(§5.16)* Migration: on an upgraded site, every appliance sub-entry's entities keep their unique id, entity id, history and statistics (a stub recorder history round-trips across the migration in the test); the old PowerPlan appliance devices are gone; exactly one repair lists every merged-away entity beside its replacement; a renamed entity keeps its name and a disabled entity stays disabled; a load whose strategy was `always` becomes `control = off` with the type's default strategy, **except** a `generic_switch` on-call load (sauna, hot_tub, other), which stays `control = auto`/`strategy = always`; a numeric priority of 22, 23, 37 and 38 becomes Lav, Normal, Normal and Høy.
...
**Keep the deprecated `device_info` pattern a while longer, since our HA floor (2026.3.0) still runs it**. *For:* one fewer migration to write right now, and the floor line does not enforce the cutoff. *Against:* the reference house already runs 2026.8.0, past the pattern's stated cutoff (Core 2026.8) - code that already does not work in the house it is deployed to is not a deferral, it is broken; and every new load added between now and whenever this is eventually fixed would need a second migration. **Decision:** fix it now, in this change, since the spec exists precisely because it stopped working.
