...
`Runtime._fetch_production_if_due` runs before `run_plan` takes the lock, in `_fetch_then_plan` (start, price entity, action) and in `_on_quarter`, each a no-op until an hour has passed since the last fetch. The preferences listener forces it and then plans. A site without a metered import register has no forecasts adapter and fetches nothing (D-0648).
...
**The runtime builds the `EventStore`.** At startup - after the price sources, before the first plan - the runtime creates one `EventStore` per site from the store section `prices.events` (D1 §7), builds an `entity` event source (`providers/events/entity.py`) for every configured event entity, reads each once and tracks it (§5.3). `runtime.EVENT_MODIFIERS` maps an add-on key to the event kind its entity announces, `{"day_type": DAY_TYPE}`, and `build_site` makes one `EntityEventSource` per add-on row that names an `entity` (with its `day_offset`). The store lives next to the raw slots in the `prices` section, `{"slots": …, "events": [...]}` (`EventStore.to_data`/`from_data`, a row that no longer reads is dropped). A state change on a source's entity polls it again, and a store that changed rebuilds the curves and runs a plan. `Inputs.events` is `EventStore.in_force(now)` in the tick and `EventStore.all()` in the plan, which prunes first. Every `PriceContext` the runtime builds answers `day_type_at` from the store in the site's zone. In v1 the only configured event entity is the `day_type` modifier's (Tempo and critical-peak days, D1 §6), and a source for the other kinds (`price_override`, `price_spike`, `reward`, `load_limit`) is one registration here plus its flow field (D-0639, D-0640).
