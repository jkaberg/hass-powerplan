...
**Strictness is strict by default** (PLAN §7 dec. 18, dec. 28, CTL-12). Three radio choices, labels of atmost five words (PLAN §7 dec. 23): **Streng (anbefalt)** (`risk` 0, **the default for every new site**), **Bruk betalte timer** (0.5) and **Fleksibel** (1.0). The longer wording ("… så lenge snittet av månedens tre høyeste timer holder seg i trinnet") is the field description, with the tariff model's own numbers as placeholders - "tre høyeste timer" is Tensio's `n = 3`, and a static label can't follow the tariff (D8 §5.15 H1). INV-9 stands: the free ride is still derived from `slack` and used when the household picks 0.5 or 1.0. `target.py::default_risk` returns 0 for every tariff model. **An existing site keeps its materialised `risk`** (INV-66) - the entry holds the number the household saw, and `risk_source` records `default` or `chosen` (D8 §9 22).
