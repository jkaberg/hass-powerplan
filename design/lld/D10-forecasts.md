...
*(D-0498.)* `async_seed` returns the uncontrolled history it folded; `core/forecasts/quantiles.HourOfWeekQuantile` folds its last 28 days into an empirical 90th percentile per local week-hour (day-hour under 3 samples) for the dashboard's reserve. Observation only: nothing plans on it.
### 5.3 Prediction
