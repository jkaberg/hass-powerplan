...
The seed cuts quarter-hour windows as far back as the 5-minute statistics reach (10 days) and hourly windows beyond. An hourly-only seed left every bin under the offer gate on the reference house. A store seeded before that is re-seeded once at startup, `forecasts.seed_version` 2 (D-0505), and a store seeded by the reader that placed rows at S once more, `seed_version` 3 (D-0687).

`uncontrolled_history` takes `meter_lag_h`, found by default. The lag is 1 when the hourly meter correlates better with Σ controlled one hour later than at the same hour (Pearson, a gain of atleast 0,10 over atleast 24 paired hours), else 0. Each register window is moved back by it before the subtraction. The AMS register on the reference house publishes each hour just after it: the water heater's 3,48 and 3,51 kWh at 22:00 and 23:00 showed on the meter as 4,40 and 4,82 kWh at 23:00 and 00:00. A window that starts before the latest first row of any load with history is skipped (`skipped`), since that load can't be taken off it - otherwise a load whose history starts later than the meter's leaks its older runs into "other usage". `lag_h` and `skipped` ride on `UncontrolledHistory` (D-0584); the seed logs both at INFO and keeps `lag_h` in the forecast store, since a lag that fires wrongly moves every bin by an hour and nothing else would show it. With rows placed where their values refer to, the reference house's register (a report every 10 s) finds 0 - r 0.97 at 0 against 0.73 at 1, 13–26 Sep - so the lag stays as the answer for a register that really publishes late, not a correction for the reader (D-0687).
...
20. The seed's placement (D-0687): HA-shaped statistics rows - hourly beyond 10 days, 5-minute within, a register reporting every 10 s - with a 3 kW load starting at 22:00 local put its energy in bin 22, not 21; a house with no production gets no negative bin; `lag_h` is 0, logged and stored; a store at `seed_version` 2 is re-seeded once.
