...
**Out of scope.** Observation in the reference house. The house is a *data source* (captured fixtures, recorder history, calibration days) and a non-blocking **house check** (§5.12), never a gate. Its checklists live here, its logs under `design/benchmarks/house/`.
...
| scenario | asserts |
|---|---|
| `reference_winter_day` | 0 windows over target; EV at 80 % by departure (07:30); tank at its 75 °C ready temperature by 06:30 within the thermostat's differential; bathrooms never below their floor (D4 §6.1: 21 °C); ≤ 1 cmd/dev/10 min Z-Wave; no unforced commitment break (INV-32) |
| `flat_price_night` (Norgespris) | no unforced commitment break across replans (INV-32); the EV's plan is one contiguous run; atmost two stops - the pause on arrival for the cheaper night (Tensio's energy charge still has `dag`/`natt` under Norgespris) and the stop when done; no start/stop churn |
| `dst_autumn` / `dst_spring` | 25/23 windows on the DST day; no duplicate/missing window; plans without gaps; `dst_spring` falls in the household's Easter week (vacation), so the tank's ready-by is dropped by design (D4 §5.12) |
| `restart_mid_window` | used_kwh continuous; no gate opens; loops restored not adopted. The winter evening with the state round-tripped through the store sections at 18:57 local, against the same evening without the restart: every closed window's kWh equal to 20 Wh, no load with more writes in the five minutes after the restart than the control run, the loops' comfort target and floor unchanged across the restart and never the device's eco setpoint, and the first tick after it not frozen (`tests/scenarios/test_phase1.py`) |
| `ble_flaps` | transient, not failure; no 0 A writes; sessions dropped == 0. The winter evening with two injected 15-min flaps on top of the simulator's own drops; the first stale ticks of a flap add no failure (a transient first); the health follows the link - `unhealthy` only while the charger's link is down or within five minutes of its return, since D4 §8 counts failures past `transient_grace_s` (15 s) and a ten-minute drop is far past it; `zero_amp_writes` 0, `sessions_dropped` 0, the charger written through the evening (`tests/scenarios/test_phase2.py`, D-0281). Every Bluetooth drop therefore raises `device_unhealthy`, and a per-profile grace in D4 §5.10 is the fix if that turns out noisy |
...
| `oven_sunday_roast` | outlier not integrated; reserve unchanged next window; the peak warning fires before the window (≥ 20 min with a confident baseline, D7 §5.4, D-0627). The winter house on a January Sunday from 12:17 with a 3 kW ceiling the roast alone exceeds (2.5 kW on 0.63 kW of base, 15:25–17:25): the oven's window breaches (`over_target ≥ 1`), the PI trim is identical in the window before and after it (INV-35), the reserve after equals the one before to 1 %, and a `peak` warning naming the oven's window is first seen before it - 3 min, the EMA's own lead (`tests/scenarios/test_phase1.py`, D-0276) |
| `legionella_expensive_week` | the cycle completes by its due date. Eight January days of spot prices under a 6 kW ceiling tight enough to compete with the EV and the floors, the tank starting cold (`tank_top_c`/`tank_bottom_c` 47/45 °C), so nothing in the plan wants the cycle early. `over_target` and `engine_failures` 0, `comfort_violation_min` 0.0 (INV-54: a bathroom never pays for the tank); the adoption anchor's `due_at` (start + 7 days) is never revised, and the cycle's one completion in the week lands at or before it - four hours eleven minutes ahead, inside the mandatory window (`due_at − 6 h`) where the plan has already lost its price vote (§5.12, `tests/scenarios/test_phase3.py`, D-0295…D-0297) |
| `heat_pump_defrost_evening` | defrost never shed, and the evening's own capacity squeeze runs next to it, blamed on neither. The whole reference house on a January night at −4 to −6 °C (below `DEFROST_BELOW_C`, `tests/sim/heatpump.py`) under a 15 kW ceiling: `engine_failures` 0, `comfort_violation_min` 0.0, six runs of the heat pump at its rated 1.5 kW for ~390 s each on the sim's own 2700 s cadence - real defrosts, not a mocked signature - and not one coincides with a shed. The evening's one real shed (stage 2, the EV's late deadline) lands 22 minutes clear of the nearest run (D-0298, D-0299) |
| `presence_away_day` | the target relaxes while away and is back at comfort when the household returns, and the floor is never crossed. An ordinary January weekday, one floor loop (the hall: comfort 22 °C, floor 18 °C - a bathroom's 24/21 would put the relaxed target exactly on the floor), no `target_kw`, so a shed would be a confound and not a finding. `HouseholdSim`'s own weekday departure and arrival is the presence signal (`Snapshot.site.presence`), not a hand-fed knob: every tick the household is `away` the hall's target reads exactly `comfort_c − away_delta` (22 − 3 = 19 °C), and the last tick, well past the arrival, is back at 22 °C (D-0300…D-0303). Arrival preheat on its own is a separate scenario |
| `dishwasher_weeknight` | one contiguous block, unshed at stages 1–3. A Wednesday evening, the whole reference house under a 15 kW ceiling tight enough to warn once the dishwasher joins (the peak warning fires, `over_target` stays 0), a real squeeze for D6 §2's `CycleReservation`. `run_once` picks 22:00 as the cheapest block that still finishes by the 07:00 ready-by (the household loads it at 19:30), and one contiguous run of `granted_w > 0` (not `measured_w`, which dips mid-programme) lasts the full ~3 h without a single shed tick (D-0304…D-0306) |
| `be_quarter_hour_rolling` | 15-min windows; rolling-12 metric; no *daily* free ride - the slack within the month is worth 1/12 (D2 §5.4, INV-9) |
| `fi_deductible` | peak kept ≤ 8 kW when it's cheap to do so |
| `es_contracted_p1_p2` | never trips; the P2 limit used at night |
| `us_srp_demand_cooling` | pre-cooling before 15:00; 30-min on-peak demand ≤ target |
| `au_solar_soak` | surplus consumed before grid |
| `zaptec_slow_trim` | a charger that accepts one change per 15 min (D4 §5.9): `over_target` = 0 through the winter week, the ceiling held by other loads and urgent sheds, never more than one non-urgent write per 900 s |
| `pv_no_battery_ev_waits` | panels, no battery, EV due 07:00 next day: charges from midday surplus whenever the export price is below the night import price; `deadline_misses` = 0 |
| `pv_battery_self_consumption` | panels and a battery, low export price: charges from surplus, discharges into the evening import, no grid charge while surplus is forecast (D5 §5.8) |
| `pv_battery_peak_shave_winter` | panels and a battery in a dark month: `peak_shave`'s reserve holds the capacity window first; `over_target` = 0 |
| `negative_price_soak` | export price below zero: soaking loads and the battery take the surplus first, nothing curtailed |
| `battery_holds_for_peak` | a dark winter day on the reference house with a 10 kWh battery whose inverter self-uses when released (`tests/sim/battery.py`'s `self_use=True`), a cheap night and a 17:00–20:00 capacity window: the battery grid-charges at night, holds through the day while the house imports and discharges in the window, SoC at 17:00 atleast `peak_shave`'s reserve, `over_target` = 0. Run on a commanded-power row (`solax_modbus`), a mode row (`goodwe`) and a floor row with no discharge command (`battery_holds_for_peak_floor`, a Powerwall's shape, D-0674). On this day no window goes over target with or without the hold, so the scenario asserts the energy given per held tick (the rows < 1e-4 kWh, a release-as-self-use > 10× that) and not an over-target window (`tests/scenarios/test_battery_hold.py`, D-0671) |
| `nl_saldering_end` | `nl_pv` across the Dutch net-metering end: no reason to shift while net metering holds (export valued at the import price), self-consumption rises after it |
| `circuit_garage_32a` | EV + sauna never exceed 32 A; a circuit breach sheds the EV only. The winter week's Saturday from 18:17, the car at 15 % under `force`, a 25 kW ceiling so the circuit is what binds, the garage `CircuitSpec` (32 A, three phases, the charger and the sauna, a clamp on the feed, `tests/builders/houses.py::GARAGE_CIRCUIT`). Before 19:00 the charger draws the whole fuse, capped by the garage; when the household lights the sauna the circuit is over its fuse for atmost three ticks while the charger yields, then the two share it and the sauna is never shed. From 20:47 for fifteen minutes an `unmetered_load` of 11 kW (a guest's car on the dumb socket and a fan heater) sits on the clamp: one `breach` event with `breach = "circuit"`, the charger at stage 4 at its floor (its stop vetoed by the plan horizon), capped by the garage, the fuse over by no more than that floor, the sauna off, the site's ladder where it was and the tank's and loops' grants on the breach tick equal to the tick before, the charger back above its floor within five minutes of the guest leaving; `over_target`, `sessions_dropped` and `zero_amp_writes` 0 (D-0284, D-0285) |
| `floor_group_rotation` | no site breach; the group only rations under real scarcity; a loop held back past `starve_seconds` is eventually admitted. A cold January evening (18 °C slabs) with all five floor loops wanting heat at once - ~5 kW of nameplate against `FLOOR_GROUP`'s 2 kW cap - under an 8 kW ceiling tight enough that the ladder rations too. `over_target`, `sessions_dropped` and `engine_failures` 0; the group inactive whenever the site isn't tight (D6 §5.6) and active atleast once; every loop whose clock reaches `starve_seconds` admitted afterwards and first in the next tick's queue; a load no group names (`ev`, `tank`) carries no clock (D-0292…D-0294) |
| `hybrid_gas_switch` | gas chosen at COP < ratio; hysteresis prevents flapping |
| `engine_exception_x3` | safe mode; all released. The runner patches the engine's step to raise for three ticks (`Fault(kind="engine_exception", at, count=3)`): safe mode on the third, every later snapshot keeps it with the site off and publishing going on, the charger back at its own maximum, the loops out of any shed and inside their band (a plan's coast setpoint isn't a shed, D7 §8, D-0272), the tank at its comfort minimum, and nothing written after |
| `savings_vs_twin` | `reference_winter_day` × 30 controlled vs the same 30 days with every load `always` on a `NoPeak` site: D11's counterfactual cost within ±10 % of the twin's actual cost, the site savings' sign right, capacity savings = the twin's fee − the controlled fee exactly (D11 §9) |
...
| `night_ev_tank_banked_floors` | a Norgespris night at a 9.7 kWh ceiling: EV (7.36 kW) and tank planned from 22:00, three floors banked; **0 ladder escalations while the measured total stays under 0.85 × the ceiling**, the projection equals `used + P_smooth × t_rem` every tick, device writes in minutes :00–:19 ≤ 40 % of the night's; 0 windows over; the EV reaches its target by departure. The same night at 4.7 kWh: the EV and the tank still meet their deadlines (D6 §9 30, 31; D-0685, D-0686) |
| `ev_plan_no_car` | the charger reports no car all night while the EV's plan still holds energy: its plan adds nothing to the projection, the ladder stays at 0 with the house at a third of the ceiling, and nothing is written to the charger (D6 §9 30, D-0685) |
...
**Battery rows.** `tests/sim/battery.py` has `self_use: bool`: a released battery balances the house as an inverter does, charging from export and discharging into import within its limits, per simulated second. Every battery row of D4 §5.9 has a fixture written from its integration's source (`tests/fixtures/captured/<row>_battery.json`, PLAN R13), and its writes for the four commands and a release are golden (`tests/golden/profiles/`), as the chargers' are.

### 5.4 Backtest

`tools/backtest.py --months 12 --tariff <tariff.json> --db /config/home-assistant_v2.db --loads loads.yaml` replays history through `WindowMeter` (reconstruction), `Evaluator` and, with `--simulate`, the full engine with simulators replacing the historical controlled loads (their historical demand becomes the simulated demand). It outputs `BacktestMetrics` and a per-window CSV, and `--compare a.json b.json` diffs two runs. `cost_energy` and `cost_counterfactual` come from the same `Accounting` class the planning loop runs (D11), fed by the replayed slots, so the backtest and the live sensor can only disagree on inputs, never on method. The phase 0 gate (HLD §9): every window under target on the reference history **in `--simulate` mode** - the historical controlled loads replaced by simulators fed their historical demand, so the controller is what's judged. The plain replay is the D2/D3 reconstruction check and lands trivially under target since the history already did. CI runs the simulator mode on synthetic houses, the recorder mode is a tool you run against your own recorder.
...
The CSV layout is one file per role (`grid_register.csv`, `grid_power.csv`, `loads/<id>.{energy,power,onoff}.csv`, optional `meta.json`), two columns each, every timestamp with a UTC offset (D-0228). The house check this produces is logged under `design/benchmarks/house/` (§5.12).
...
`test_purity.py`: AST-walk every module under `core/`, and any `import homeassistant` fails. `test_single_writer.py`: only `writegate.py` performs device writes, and a provider may invoke a read-only response action. Concretely, grep `hass.services.async_call` - allowed only in `writegate.py`, `notifications.py` (notify/persistent_notification), `providers/prices/nordpool_action.py` (core Nord Pool's `get_prices_for_date`) and `providers/schedules/ha_schedule.py` (the `schedule` integration's `get_schedule` - a bound helper's weekly windows aren't in its state or attributes, so this action is the only read path), all registered `SupportsResponse.ONLY` - plus an AST walk of those two files asserting every call site passes `return_response=True`, so the exemption can't become a write; `hass.states.get`/`async_all` - allowed only in `runtime.py` and `providers/`. See D-0080, D-0300.
...
`fi_linear`, `es_contracted` and `fr_tempo` share one builder, `tests/builders/houses.py::_market_house` - `be_quarter`'s body with the site, tariff, currency and price as arguments (`nl_pv` and `be_quarter` keep their own, so their baselines don't move). Each has `tests/benchmark/houses/<name>.py` and a `smoke` baseline. `fi_linear`: Helsinki, TN 400 V 3×25 A, the tehomaksu shape (8 kW free on the month's highest hour, 2.50 EUR/kW/month) as a benchmark-only rule, `SPOT_LIKE` in EUR. `es_contracted`: Madrid, TT 400 V 3×25 A, `es/2_0td` filled P1 5.75 kW, P2 9.2 kW, `SPOT_LIKE` in EUR. `fr_tempo`: Paris, TT 400 V 3×25 A, `fr/kva` filled 12 kVA; the energy price is EDF's Tempo grid (HC 22–06, the day from 06:00) as a `day_type` add-on on a `FLAT` source at 0 (`PriceSim.flat_price`), and `tests/sim/tempo.py::TempoSim` announces the colours - RTE's 22 red and 43 white days per Tempo year, red on winter weekdays, white never on a Sunday, ranked by the house's own noon temperature, each announced at 10:40 the day before. The runner holds what was announced in an `EventStore` (`House.announcer`): `day_type_at` answers from it and `Inputs.events` carries what's in force, and a house without an announcer gets an empty store, byte-identical to before. The houses are three phase because `nordic_detached`'s loads are (D-0641).

**`us_demand`:** Phoenix (`America/Phoenix`, no DST), US split 240 V; SRP E-27's summer demand shape - the highest 30-minute on-peak average of the month, on-peak 14:00–20:00 on weekdays May–October, three bands ($11.90 to 3 kW, $19.97 to 10 kW, $36.05 above) - as a benchmark-only rule; `WeatherSim` with Phoenix's monthly climate normals as a table (NOAA 1991–2020, Phoenix Sky Harbor) in place of Trondheim's; the air-to-air heat pump cooling; a July fortnight. `us_srp_demand_cooling` asserts the pre-cooling before 15:00 and the on-peak 30-minute demand at or under the target (D-0642).

`us_demand` is `_market_house` with the Phoenix normals as `WeatherSim`'s `monthly_*_c` tables, the SRP rule, a US split 240 V 200 A service, the EV on two 240 V legs (`_ev(phases=2, w_per_amp=240)`) and the heat pump cooling at 24 °C with **no away setback** (`follow_presence` off - a Phoenix house holds its air conditioning, and with the setback the planner can't know when the house returns, so nothing is pre-cooled). The scenario runs two August weekdays: no on-peak 30-minute window over 5 kW, and on each weekday the unit's energy 10:00–14:00 exceeds its energy 14:00–20:00 - it cools before the window and coasts through it inside its band. It doesn't bank below the target, the demand charge isn't in the energy price `heat_capacitor` banks on (D5 §5.7). Smoke baseline: fee 31.05 USD at 2.61 kW in October, January outside SRP's season (D-0644).
...
`au_solar@1` - panels and a battery - has its own baseline, as does the metric `self_consumption` (self-consumed production ÷ production, per month and total). `nl_pv` crosses the Dutch net-metering end inside `y2026_27`: before it export is valued at the import price, after it at 50 % of the bare supply price, the legal floor to 2030 (Rijksoverheid). That's a version bump of the house (`nl_pv@2`) with a `design/benchmarks/CHANGELOG.md` line.

`au_solar@1` is `_market_house` in Sydney: one phase of 230 V at 63 A, the BOM 1991–2020 normals, the sun at −33.87°, 6.6 kWp, a 13.5 kWh battery with grid charging on, `SOLAR_GLUT` in AUD exported at spot, and a benchmark-only tariff with no demand charge and a flat 0.12 AUD/kWh network charge (`AU_WHOLESALE`), on 60-minute windows. `nl_pv@2` opts into the runner's forecast and exports at the import price (`"net"`) until the switch and at half the spot (`"half_spot"`) after (`House.export_switch`). `self_consumption` is `(production − export) / production`, per month and in total, from the roof's simulator and the grid total per tick. It's only recorded for a house with panels, so the other baselines stay byte-identical, and its tolerance is `not_less` (D-0657). `fi_linear` keeps its tariff model as a benchmark house: the house tests the shape, not a shipped bill.
...
Baselines live in `tests/benchmark/baselines/<house>.json` with the build hash and the WP that set them - one file per house, one result per tier under `results`, the tolerances next to them (`tests/benchmark/baseline.py`; `tools/benchmark.py --update-baseline --since WPn.m` writes it, `--compare` reads it). Tolerances per metric: `over_target`, `comfort_violation_min`, `deadline_misses`, `legionella_lapses`, `sessions_dropped` → `zero` once the enabling WP has merged (`not_worse` before); `fee`, `cost_energy` → `not_worse` (a 0 % tolerance in the direction of more money); `savings` → `not_less` (the same 0 %, in the direction of less money saved); `kwh_shifted` → informational; `writes` → `pct 10`; `tick_p95_ms`, `plan_p95_ms` → `abs` thresholds from §5.1. `tools/benchmark.py --compare` fails on any breach and prints the table for the PR description. A PR whose change *legitimately* moves a metric - a new load type under control, a default changed on purpose - updates the baseline in the same PR and adds one line to `design/benchmarks/CHANGELOG.md` (WP, metric, old → new, why). Improvements update the baseline too, so the ratchet holds both ways. The house spec and the year are versioned (`nordic_detached@1`, `y2026_27@1`), and changing them is a baseline reset with the same changelog line.

### 5.12 House checks (non-blocking)

What the reference house contributes, whenever a build runs there: captured fixtures (`capture_fixture.py`), recorder history for the backtest (§5.4) and for re-fitting the fiction (§2), observe-mode calibration days for D11's shadows, and the human checks no simulator can do ("a newcomer adds a floor loop in under two minutes"). Each is a checklist under `design/benchmarks/house/` with the build hash and the numbers. A failed house check opens an issue and, where it exposes a gap in the simulator, a change to the house spec - it never blocks a phase.
...
- **No test touches the network.** Every source adapter (D13 §5.11) runs on captured documents under `tests/fixtures/tariff_sources/<key>/` with a `COMMIT` or `CAPTURED` file naming where and when each was taken, and a setup test fails on any HTTP call (D13 §19 14). The flow tests use fake sources, one per tier, to walk the ladder (D13 §19 15).
- **The nightly live job** (`.github/workflows/tariff-sources.yml`, a schedule, never in a household's HA and never in the PR suite) is `tools/tariff_canary.py`: it fetches every adapter's live endpoint with its named User-Agent, runs its contract test (the fields it reads, the paths) and cross-checks each company across the tiers that exist (fri-nettleie against Strømpriseridag, Digin with the maintainer's own keys, Tensio's page). It exits 1 on a finding, and every run rewrites one issue, "Tariff test dashboard", with what it found (D-0559, D-0684). `tests/core/invariants/test_no_network.py` pins the harness's socket guard.
...
Test artefacts under `tests/fixtures/`, `tests/golden/` and `tests/benchmark/baselines/` are committed. Benchmark and backtest outputs are gitignored except `design/benchmarks/CHANGELOG.md` (baseline changes), `design/benchmarks/nightly/` (the nightly table, the last 90 days) and `design/benchmarks/house/` (house checks).
...
4. Golden files for the v1 rule templates.
5. The Snapshot schema golden.
6. Perf test skeletons with thresholds: `tests/perf/tick_budget.py`/`plan_budget.py`, a synthetic 20-load engine (not the reference house's twelve), gating tick P95 < 50 ms and plan P95 < 500 ms - measured well inside both (≈ 6.0 ms, ≈ 14.6 ms, D-0328).
7. `capture_fixture.py` round-trip: capture → load as `DeviceView` → profile match reproduces the documented result.
8. The reference benchmark: `nordic_detached` × `y2026_27` runs deterministically (two runs, same seed → byte-identical `BenchmarkResult`); the year has 365 days and exactly 8 760 hours (the autumn +1 and the spring −1 cancel), 92/96/100-slot days on the DST dates; every generator parameter has a `source` string; `smoke` ≤ 1 min, `month` ≤ 5 min and `full` ≤ 2 h on the CI runner; ≥ 500 ticks/s (2 000 reported as a target metric). **Not met:** `smoke` measures ≈ 151 ticks/s and ≈ 800 s wall - diffuse, architecturally load-bearing cost (`dataclasses.replace` across 40+ call sites, the simulator's per-value deterministic RNG, pervasive tz-aware datetime arithmetic, all three required elsewhere in this design) and not one fix (D-0328). Item 6's latency budgets are a different property and are met.
9. Baseline machinery: a metric outside tolerance fails `--compare` with the offending row, `zero` tolerances are enforced, and a baseline update without a `design/benchmarks/CHANGELOG.md` line fails a lint check.
10. `e2e` day: the integration set up through its real flows against `fake_house`; every entity in D8 §5.5 exists; writes arrive with `blocking=True` (the executor's test; the day itself runs in observe); the pure runner's same day agrees - closed windows one for one, `over_target` and the capacity fee equal, writes both zero; the phase 1 gate's projection p95 and warning leads as §5.10 defines them (`tests/e2e/test_e2e_day.py`).
11. Uncontrolled loads in the house spec (types not implemented yet) run on their own logic and are metered into `uncontrolled`, and the baseline's `controlled_share` matches the build.
12. `tools/preset_age.py` lists exactly the facts verified more than 6 months before a given date.
...
**Shipped tariff adapters in every test.** *For:* nothing to register. *Against:* a test would then depend on every adapter's registration side effects. **Decision:** the shipped adapters are out of the registry during every test (`tests/conftest.py`), and a test that wants one registers it and serves its captured documents - `tests/builders/tariff_sources.py` (`fri_http()`: fri-nettleie's archive at a pinned commit and NVE's county list), or `Http._download` replaced, which keeps `Http`'s cache and release as shipped (D-0566).

**Test the hold with unit tests only, no inverter simulator.** *For:* D4 §9 40 and D5 §9 28 pin the writes and the plan, and a simulator is more code. *Against:* the defect lives between the layers - each unit is right on its own (the plan idle, the profile's release right), and only a battery that self-uses when released shows the evening arriving empty. §11's rule is simulators over mocks for anything physical. **Decision:** `tests/sim/battery.py` has the inverter's self-use, and `battery_holds_for_peak` runs on each command shape that can grid-charge.
