...
| `oven_sunday_roast` | outlier not integrated; reserve unchanged next window; peak warning fires before the window (≥ 20 min with a confident baseline, D7 §5.4; D-0627). **As asserted (`tests/scenarios/test_phase1.py`, D-0276):** the winter house on Sunday 2027-01-17 from 12:17 with a 3 kW ceiling, which the roast alone (2.5 kW on 0.63 kW of base, 15:25–17:25) exceeds; the oven's window breaches (`over_target ≥ 1`), the PI trim is identical in the window before and the window after it (INV-35), the reserve in the window after equals the one before to 1 %, and a `peak` warning naming the oven's window is first seen before it (measured: 3 min, the EMA's own lead; 21 min until v0.3.5, when the plans' energy was counted - D-0627) |
...
| `battery_holds_for_peak` | a dark winter day on the reference house with a 10 kWh battery whose inverter self-uses when released (`tests/sim/battery.py`'s `self_use=True`), cheap night, a 17:00–20:00 capacity window: the battery grid-charges at night, holds through the day while the house imports, and discharges in the window; the SoC at 17:00 is at least `peak_shave`'s reserve, `over_target` = 0. Run on a commanded-power row (`solax_modbus`) and a mode row (`goodwe`); a floor row (`tesla_fleet`) joins in WP7.12. *(D-0671:)* on this day no window goes over target with or without the hold. The old release (the simulator's `hold_as_self_use`) spends about sixty times the energy per held tick and loses most of its holds to replans, so the scenario asserts the energy given per held tick (new rows < 1e-4 kWh, old > 10× new), not an over-target window (`tests/scenarios/test_battery_hold.py`) |
