...
| Lifecycle | `async_setup_entry`: restore stores → release every load → restore comfort setpoints → provision profiles → first tick → forward platforms (INV-48). `async_unload_entry`: stop engine → release every load → flush the store. A shed that is not undone on unload is a bug. |
...
- **INV-26** Letting go undoes the shed. `release()` runs on mode→off, site→off, unload, startup - and ignores dwell clocks because it is not a control action.
- **INV-27** A comfort target (`komfort_c`) comes from configuration, never from the device. A thermostat in eco reports its eco setpoint; reading it as the target closes a loop with no external cause.
...
- **INV-29** Heat pumps: setpoint bounded to `komfort ± band` then the device's own limits; band hard-capped and rejected not clipped; startup **restores** the comfort value, never adopts what it finds; no upward move within one dwell of a restore; never shed during defrost; the mains switch is never actuated.
...
- **INV-66** Derived values are **materialised** into the subentry at setup. A later change to a derivation table never silently changes an existing load; the user re-derives on request.
...
- **INV-48** Startup order: restore stores → release every load → restore comfort setpoints → provision profiles → first tick. A load never inherits the mode it was left in.
...
- **INV-50** Entity ids and attribute names are stable across versions; attributes are English; translations cover Norwegian and English at launch.
