...
|---|---|
| Now | anything that needs your attention - PowerPlan's repairs and a meter that lags or is silent, each with one line of why and a button; nothing when all is well; this hour's usage against its limit; your electricity price now, today and tomorrow - with a fixed price such as Norgespris, also what it would be without it and what it has saved this month; tomorrow's half of the curve fills in when its prices are in; when this hour's price is not known, why, and a button to fetch it again; the whole house for the next 24 hours, hour by hour, with a button to plan again; one row per appliance with its status and its next start - tap it for its controls, its plan and why, and from there its own page; your capacity step and its monthly fee; this month's cost, split into energy and the capacity fee. Under it, one line each on what PowerPlan did: what it saved and from where, which capacity step you would be on without it, what your appliances paid per kWh against what they would have paid, and, only when something happened, deadlines missed, time below comfort and hours over the target. In the month PowerPlan starts, a last line says the energy is counted from that day while the capacity fee is the whole month's. Then a bar for each day of the month. A day that holds a one-off sum, such as the month's capacity fee, is cut off at the top with its amount written above it, so the ordinary days stay readable |
| History | a summary of the period - cost, savings (once PowerPlan has a reference to compare with), energy from the grid and your capacity basis; your usage hour by hour or day by day against your power target, with a link to the Energy dashboard; each day's highest hour and whether it counts toward your capacity step; cost and savings per appliance, and, for the current month on a wide screen, the kWh each one moved; what happened. The period picker at the bottom sets every graph, as on the Energy dashboard |
...
- **The capacity gauge** shows this hour's usage against its limit. The pale part of the arc and the small tick show where the hour is expected to end. A label at the top appears only when the hour needs you: *Tight*, *Critical* or when a peak is expected. The three figures at the bottom are the expected total, the minutes left of the hour and the power still available. Tap it for the hour's history.
