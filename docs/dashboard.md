...
| view | shows |
|---|---|
| Overview | the capacity window right now, the home's controls (on/off, presence, target), where the power goes, the next 24 hours, what is coming up, this month's cost and savings, advice, and anything that needs your attention |
| Plan | the next 48 hours, the price now and whether tomorrow's prices are in, each appliance's status, and the planned runs as a calendar |
| Appliances | one section per appliance, most important first: its control, its status, its own settings (charge to, comfort, ready by, hours per day), and this month's cost and savings |
| History | cost and savings, the capacity windows against their ceiling, the capacity level, energy and cost per appliance, and what happened; the period picker at the bottom sets every graph |

The planned runs are also in Home Assistant's *Calendar* panel as **Planned runs**.

Two cards are PowerPlan's own:

- **The timeline** shows the next 24 hours (48 on *Plan*). The bars are what each appliance is planned to use, the shaded area is the rest of the house, the dashed red line is the capacity limit and the blue line is the price. Everything is in kW, so a quarter-hour and an hour read the same way. A grey band marks prices that are estimated, not yet published. Tap a slot to see its energy, price and cost. Tap a name in the legend to hide it.
...
If you have **more than one PowerPlan home**, each gets its own four tabs, named after the home, for example *Cabin · Overview*. To show one home only, add its entry id:
...
strategy:
  type: custom:powerplan
  hidden_views: [history]          # overview, plan, loads, history
  hidden_cards: [calendar, repairs] # any card type
```
